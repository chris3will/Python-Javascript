...
def creat_image_dataset(pic_dir):
    ...
    for class_dir in os.listdir(pic_dir):
        if not os.path.isdir(os.path.join(pic_dir, class_dir)):
            continue
        print("dealing with ",str(class_dir))   
...
def show_name(img_path, predictions, key_dic):
    ...
    ttFont = ImageFont.truetype("simhei.ttf", 5)
    print(predctions)
...
if __name__ == '__main__':
    ...
    #第一个参数是数字，代表模式
    #1 最完全模式1/ 输入样本集/ 模型地址/ 待检测数据/ 匹配字典/
    #2 训练模式2/ 输入新样本集/ 新模型地址/
    #3 验证模式3/ 输入待检测数据/ 模型地址/ 匹配字典/
    print(sys.argv)
    if(len(sys.argv)>1):
        if(str(sys.argv[1])=='1'):
            ...
            print("该模型准确率为{:.2f}".format(mlp.score(x_test,y_test)))
            #先不管结果如何勒直接进行模拟
            train_model(faces_data,face_name,sys.argv[3],2000)
        else:
